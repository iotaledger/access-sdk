...
int auth_utils_write(auth_ctx_t *session, const unsigned char *msg, unsigned short message_length) {
  ...
  int n = tcpip_write_socket(session->sockfd, buffer, buffer_length);
...
int auth_utils_read(auth_ctx_t *session, unsigned char **msg, unsigned short *message_length) {
  ...
  tcpip_read_socket(session->sockfd, buffer, CHARS_TO_READ);
  ...
  tcpip_read_socket(session->sockfd, encrypted_msg_buffer + 3, encrypted_data_length);

  tcpip_read_socket(session->sockfd, received_mac, MAC_HASH_L);
