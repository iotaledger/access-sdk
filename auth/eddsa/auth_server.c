...
int auth_server_compute(auth_ctx_t *session) {
  ...
  // Server receives e from Client.
  int read_message = tcpip_read_socket(session->sockfd, received_dh_public, DH_PUBLIC_L);
  ...
  int write_message = tcpip_write_socket(session->sockfd, writeBuffer, SIZE_OF_WRITE_BUFFER);

  // Server receives ( kc || sc )
  int read_second_message = tcpip_read_socket(session->sockfd, readBuffer, PUBLIC_KEY_L + SIGNED_MESSAGE_L);
